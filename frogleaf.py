...
i.e. the leaves do not change their positions once they fall in 
the river.
...
integers and integer X, returns the earliest time when the frog 
...
def solution(X, A):
    ...
    for sec, x in enumerate(A):
        if x <= X:
            if count_arr[x] == -1 or sec < count_arr[x]:
                count_arr[x] = sec
