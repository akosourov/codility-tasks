...
of successive nucleotides in the sequence. Each nucleotide 
has an impact factor, which is an integer. Nucleotides of 
types A, C, G and T have impact factors of 1, 2, 3 and 4, 
...
S = S[0]S[1]...S[N-1] consisting of N characters. There are M 
queries, which are given in non-empty arrays P and Q, each 
consisting of M integers. The K-th query (0 ≤ K < M) requires you 
to find the minimal impact factor of nucleotides contained in the 
DNA sequence between positions P[K] and Q[K] (inclusive).

For example, consider string S = CAGCCTA and arrays P, Q such 
that:
...
G and C (twice), whose impact factors are 3 and 2 respectively, 
so the answer is 2.
...
def solution(S, P, Q):
    len_s = len(S)
    count_A_prefix = [0] * (len_s + 1)
    count_C_prefix = [0] * (len_s + 1)
    count_G_prefix = [0] * (len_s + 1)
    for i, el in enumerate(S):
        # copy previos value
        count_A_prefix[i+1] = count_A_prefix[i]
        count_C_prefix[i+1] = count_C_prefix[i]
        count_G_prefix[i+1] = count_G_prefix[i]

        # count elements from left to i
        if el == 'A':
            count_A_prefix[i+1] += 1
        elif el == 'C':
            count_C_prefix[i+1] += 1
        elif el == 'G':
            count_G_prefix[i+1] += 1
    ...
    for i in range(len_p):
        ...
        if count_A_prefix[right] > count_A_prefix[left]:
            res[i] = 1
        elif count_C_prefix[right] > count_C_prefix[left]:
            res[i] = 2
        elif count_G_prefix[right] > count_G_prefix[left]:
            res[i] = 3
        else:
            res[i] = 4
